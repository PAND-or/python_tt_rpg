...
"""
При запуске скрипта запросите у игрока имя и класс, который вы описали в readme (input). Проверьте, что игрок правильно 
указал класс, если вместо класса было введено недопустимое значение, попросите ввести класс еще раз.

Напишите приветственное слово главному герою (print). И подскажите, что, используя команду ‘help’ 
он может получить всю необходимую информацию. Используйте форматирование строк, чтобы достойно поприветствовать вашего 
героя, обратитесь к нему по имени и в соответствии с классом/расой. 

По команде ‘help’ на экран необходимо вывести список доступных игроку команд с кратким описанием каждой команды. 
Для первой версии игры достаточно будет небольшого количества команд 
(“history”, “help”, “train”, “exit”, и др. по вашему желанию). 

Игра должна работать в бесконечном цикле, принимая команды игрока через консоль, пока не будет введена команда ‘exit’. 
Позже вы добавите и другие условия завершения игры - победа и смерть.  Вы можете реализовать обработку команд игрока 
с помощью условного оператора if elif else (например, if action == ‘your_command’). Выводите предупреждение, если 
введена неизвестная команда. 

Реализуйте возможность прочитать историю игры (лор) по команде ‘history’. Выведите описание героя, врагов, оружия, 
артефакта и всего прочего, что вы описали в readme.

Попробуйте реализовать тренировочный бой (команда ‘train’) с использованием цикла. 

Запросите у игрока кол-во очков здоровья противника и силу удара героя. Убедитесь, что удар героя не превышает 
количества очков здоровья противника, если это условие ложно, присвойте силе удара значение равное 25% здоровья врага.

Герой сражается с условным противником, нанося ему по удару за ход, отнимая очки здоровья (hp) у противника, 
пока его hp не достигнет 0. В тренировочном бою ваш соперник не может защищаться или атаковать в ответ. В конце 
каждого хода (итерации цикла) выводите текущее состояние здоровья героя и противника. Когда hp противника достигнет 
нуля, бой заканчивается победой героя. Поздравьте его! 
*Попробуйте усложнить схему поединка, добавив ответные ходы противника.

Добавьте форматирование текста при выводе в весь получившийся код. Например, одинаковые отступы, выравнивание 
по правому краю или центру и др. на ваш выбор. 
